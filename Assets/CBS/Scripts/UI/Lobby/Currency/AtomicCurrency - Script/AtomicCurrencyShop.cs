...
namespace CBS.UI
{
    public class AtomicCurrencyShop : MonoBehaviour
    {
        [SerializeField]
        private Transform RootContent;

        [SerializeField] private AtomicCurrencyShopTab[] tabs;

        private CurrencyPrefabs Prefabs { get; set; }
        private ICurrency Currencies { get; set; }
        private List<CBSCurrencyPack> _packs;
        private List<AtomicCurrencyPackItem> _currencyPackItems = new(64);
        ...
        private void SpawnPacks()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                var currencyPack= packObj.GetComponent<AtomicCurrencyPackItem>();
