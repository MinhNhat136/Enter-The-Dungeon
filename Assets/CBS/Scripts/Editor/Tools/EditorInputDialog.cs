...
namespace CBS.Editor
{
    public class EditorInputDialog : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            {
                OnSubmitAction?.Invoke(inputText);
                onOKButton?.Invoke();
                shouldClose = true;
            }
        ...
        /// <returns></returns>
        public static string Show(string title, string description, string inputText, Action<string> onSubmit, string okButton = "OK", string cancelButton = "Cancel")
        {
            ...
            var maxPos = new Vector2(Screen.width / 2, Screen.height / 2);
            ...
            window.OnSubmitAction = onSubmit;
            //window.ShowPopup();
            window.ShowModal();
