    ...
    public class AddBattlePassInstanceWindow : EditorWindow
    {
        ...
        void OnGUI()
        {
            using (var areaScope = new GUILayout.AreaScope(new Rect(0, 0, 400, 700)))
            {
                ...
                EditorGUILayout.HelpBox("Unique ID for the battle pass. Don't use special characters or spaces. ID must be in lower case", MessageType.Info);
                ...
                if (GUILayout.Button("Save"))
                {
                    ...
                    if (TextUtils.ContainSpecialSymbols(EntereID))
                        return;
                    if (TextUtils.ContainUpperCase(EntereID))
                        return;
